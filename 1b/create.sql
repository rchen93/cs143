...
CREATE TABLE MovieDirector (mid int, did int, PRIMARY KEY(mid));
CREATE TABLE MovieActor (mid int, aid int, role varchar(50), PRIMARY KEY(mid));
CREATE TABLE Review (name varchar(20), time timestamp, mid int, rating int, comment varchar(500), PRIMARY KEY(name));
CREATE TABLE MaxPersonID (id int, PRIMARY KEY(id));
CREATE TABLE MaxMovieID (id int, PRIMARY KEY(id));